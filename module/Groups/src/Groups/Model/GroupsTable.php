...
{ 
	...
	public function getNewsFeedsAPI($user_id,$type="",$group_id="",$activity="",$limit="",$offset=""){
        ...
        switch($type){
            case 'status':
                $sql = 'SELECT group_discussion_id as event_id,group_discussion_added_timestamp as update_time,if(group_discussion_id,"New Status","") as type,user_id,user_given_name,user_profile_name,profile_photo,user_fbid,group_title,group_seo_title,group_photo_photo,group_id  FROM  y2m_group_discussion INNER JOIN y2m_user ON y2m_user.user_id = y2m_group_discussion.group_discussion_owner_user_id INNER JOIN y2m_group ON y2m_group_discussion.group_discussion_group_id = y2m_group.group_id INNER JOIN y2m_group_photo ON y2m_group_photo.group_photo_group_id = y2m_group.group_id LEFT JOIN y2m_user_profile_photo ON y2m_user.user_profile_photo_id = y2m_user_profile_photo.profile_photo_id WHERE group_discussion_group_id IN (SELECT user_group_group_id FROM y2m_user_group WHERE user_group_user_id = '.$user_id.' AND user_group_status = "available" ) AND group_discussion_status = "available" AND (y2m_user.user_status = "live" OR y2m_user.user_status = "not activated") AND group_status = "active"';
                if($group_id!=''){
                    $sql.=' AND group_id = '.$group_id;
                }
                if($activity == 'interactions'){
                ...
                break;
            case 'media':
                $sql = 'SELECT group_media_id as event_id,media_added_date as update_time,if(group_media_id,"New Media","") as type,user_id,user_given_name,user_profile_name,profile_photo,user_fbid,group_title,group_seo_title,group_photo_photo,group_id FROM  y2m_group_media INNER JOIN y2m_user ON y2m_user.user_id = y2m_group_media.media_added_user_id INNER JOIN y2m_group ON y2m_group_media.media_added_group_id = y2m_group.group_id INNER JOIN y2m_group_photo ON y2m_group_photo.group_photo_group_id = y2m_group.group_id LEFT JOIN y2m_user_profile_photo ON y2m_user.user_profile_photo_id = y2m_user_profile_photo.profile_photo_id WHERE media_added_group_id IN (SELECT user_group_group_id FROM y2m_user_group WHERE user_group_user_id = '.$user_id.' AND user_group_status = "available" ) AND media_status = "active"  AND (y2m_user.user_status = "live" OR y2m_user.user_status = "not activated") AND group_status = "active"';
                if($group_id!=''){
                    $sql.=' AND group_id = '.$group_id;
                }
                if($activity == 'interactions'){
                ...
                break;
            case 'event':
                $sql = 'SELECT group_activity_id as event_id,group_activity_added_timestamp as update_time,if(group_activity_id,"New Activity","") as type,	user_id,user_given_name,user_profile_name,user_fbid,profile_photo,group_title,group_seo_title,group_photo_photo,group_id FROM  y2m_group_activity INNER JOIN y2m_user ON y2m_user.user_id = y2m_group_activity.group_activity_owner_user_id 	INNER JOIN y2m_group ON y2m_group_activity.group_activity_group_id = y2m_group.group_id INNER JOIN y2m_group_photo ON y2m_group_photo.group_photo_group_id = y2m_group.group_id LEFT JOIN y2m_user_profile_photo ON y2m_user.user_profile_photo_id = y2m_user_profile_photo.profile_photo_id WHERE group_activity_group_id IN (SELECT user_group_group_id FROM y2m_user_group WHERE user_group_user_id = '.$user_id.' AND user_group_status = "available") AND group_activity_status = "active" AND (y2m_user.user_status = "live" OR y2m_user.user_status = "not activated") AND group_status = "active"';
                if($group_id!=''){
                    $sql.=' AND group_id = '.$group_id;
                }
                if($activity == 'interactions'){
                ...
                break;
            default :
                $sql = 'SELECT group_activity_id as event_id,group_activity_added_timestamp as update_time,if(group_activity_id,"New Activity","") as type,	user_id,user_given_name,user_profile_name,profile_photo,user_fbid,group_title,group_seo_title,group_photo_photo,group_id FROM  y2m_group_activity INNER JOIN y2m_user ON y2m_user.user_id = y2m_group_activity.group_activity_owner_user_id 	INNER JOIN y2m_group ON y2m_group_activity.group_activity_group_id = y2m_group.group_id INNER JOIN y2m_group_photo ON y2m_group_photo.group_photo_group_id = y2m_group.group_id LEFT JOIN y2m_user_profile_photo ON y2m_user.user_profile_photo_id = y2m_user_profile_photo.profile_photo_id WHERE group_activity_group_id IN (SELECT user_group_group_id FROM y2m_user_group WHERE user_group_user_id = '.$user_id.' AND user_group_status = "available") AND group_activity_status = "active" AND (y2m_user.user_status = "live" OR y2m_user.user_status = "not activated") AND group_status = "active"';
                if($group_id!=''){
                    $sql.=' AND group_id = '.$group_id;
                }
                if($activity == 'interactions'){
