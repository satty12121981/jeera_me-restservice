											...
											if($comment_id){												 
												...
												foreach($joinedMembers as $members){ 
													if($members->user_group_user_id!=$identity->user_id){
														...
														$this->UpdateNotifications($members->user_group_user_id,$msg,2,$subject,$from,$identity->user_id,$comment_id);
													}
												}
											}
							...
							if(!empty($comments_details)){
								foreach($comments_details as $list){
									...
									if(!empty($like_details)){  
										$liked_users = $this->getLikeTable()->likedUsersWithoutLoggedOneWithFriendshipStatus($commentSystemTYpe->system_type_id,$refer_id,$identity->user_id,2,0);
